...
object OffsetUtils {
  ...
  def getOffsetsMap(groupId: String, topic: String): mutable.Map[TopicPartition, Long] = {
    ...
    //2.编写sql
    val sql:String = "select partition1,offset1 from t_offset where groupid = ? and topic = ?"
    //3.获取ps
    ...
    while(rs.next()){
      val partition: Int = rs.getInt("partition1")
      val offset: Int = rs.getInt("offset1")
   ...
   * `partition1` int(11) NOT NULL,
   * `groupid` varchar(255) NOT NULL,
   * `offset1` bigint(20) DEFAULT NULL,
  ...
  def saveOffsets(groupId: String, offsets: Array[OffsetRange]) = {
    ...
    val conn: Connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/covid19?characterEncoding=UTF-8","root","root")
    //2.编写sql
    val sql:String = "replace into t_offset (topic,partition1,groupid,offset1) values(?,?,?,?)"
    //3.创建预编译语句对象
    val ps: PreparedStatement = conn.prepareStatement(sql)
    //4.设置参数并执行
    for (o<-offsets){
